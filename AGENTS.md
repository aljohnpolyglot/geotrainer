...
- Learn priority defaults to Random. Familiar places samples near encountered locations, while Least exposure first balances eligible countries, fills zero-exposure local regional pools when available, and then targets the largest geographic gap from saved coordinates; keep the rule consistent for single-country, mixed-country, continent, and world collections. In multi-country Least exposure searches, advance after every unsuccessful lookup and cycle remaining countries from least to most exposure; recover Kosovo only when Google omits a country code near a bundled Kosovo seed, and keep sparse-seed microstate searches tight instead of sampling their coarse bounds.
