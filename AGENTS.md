...
- The Official / Mixed / Contributor imagery selector defaults to Official and filters newly generated Custom Learn and Play locations plus panorama choices made in Explore Map; it does not alter Meta, saved locations, History, or Review.
- The indoor-coverage dropdown defaults to Outdoors only and offers Mixed indoor/outdoor coverage for newly generated Custom Learn and Play locations plus panorama choices made in Explore Map; Google does not provide a reliable indoor-only Street View search. Meta, saved locations, History, and Review remain unchanged.
...
- Keep separate region and city dropdowns with their own removable pills beneath explicitly selected countries in both Study and Play. No region selection means the normal country-wide pool; no city selection within a selected region means all its available city seeds. City choices narrow that region, removing its last city restores the region pool, and removing a region clears its cities. New Learn setup must not reuse temporary focused-country collections as selectable base collections. Load their local per-country GeoNames data lazily, show localized names when available, sort regions and alphabetical cities with the interface locale, offer population-importance city ordering, and treat a region’s All choice as its available city seeds rather than a complete administrative polygon.
- Keep uploaded Map Maker JSON pools local to the device; Learn and Play sample their exact listed panoramas or nearby coordinates, while saved attempts and Review progress remain compatible with cloud sync. Keep Learn's Upload and URL sources separate, show the loaded source name, and return detected GeoTrainer geographic-pool JSON to editable Custom selections. AI-described pools must also return to editable Custom selections and validate suggestions against local country, region, and city data.
