...
- Learn priority defaults to Random. Familiar places samples near encountered locations, while Least exposure first balances eligible countries and then targets the largest geographic gap from saved coordinates; keep the rule consistent for single-country, mixed-country, continent, and world collections.
...
- In Uploaded map Learn, Previous returns through locations already drawn in the current session; Next moves forward through that history before drawing another map location.
...
- Keep optional region and city pools beneath explicitly selected countries in both Study and Play. Load their local per-country GeoNames data lazily, list regions alphabetically, offer population-importance and alphabetical city ordering, and treat a region’s All choice as its available city seeds rather than a complete administrative polygon.
