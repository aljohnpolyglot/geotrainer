...
Méta sélectionne uniquement les leçons non terminées. Dans la configuration, **Général** continue avec une leçon non terminée au hasard, tandis que **Parcourir** recherche dans le catalogue traduit et lance la leçon choisie. Parcourir peut afficher toutes les leçons, celles à faire ou celles terminées ; les lignes terminées sont visuellement marquées et ne peuvent pas être relancées. Une fois les 359 achevées, son option d’apprentissage est grisée et ne peut plus être sélectionnée.
