...
import { BookmarkLocation, Collection, Environment, LearnedMeta, LocationResult, PanoramaSource, SamplingMode, StudyVisit, TrainerLocation, UrbanLevel } from '../types';
...
import { moveImportedHistory, pickImportedLocation, type ImportedMapHistory } from '../services/importedMap';
...
export function useStudyMode(ctx: any) {
  ...
  const fetchNextLocation = useCallback(async () => {
    ...
    try {
      const result = importedMapId
        ? await pickImportedLocation(importedMapId, new Set(recentStudyPanosRef.current), controller.signal)
        : await defaultLocationGenerator.findRandomLocation(collection.countryCodes, controller.signal, (message) => { if (isLatestRequest(requestId, latestGenerationRequestRef.current)) ctx.setStatusMessage(message); }, { environment: studyEnvironmentRef.current, urbanLevel: studyUrbanLevelRef.current, samplingMode: studySamplingRef.current, panoramaSource: studyPanoramaSourceRef.current, allowInteriors: studyAllowInteriorsRef.current }, { requestId, collectionId: collection.id, excludedPanoIds: new Set(recentStudyPanosRef.current), requireNavigation: true });
      ...
      if (importedMapId) { uploadedHistoryRef.current = moveImportedHistory(uploadedHistoryRef.current, 'next', result); setCanPreviousUploaded(uploadedHistoryRef.current.index > 0); }
  ...
  const handleStartStudy = (settings: { collectionId: string; countryCodes?: string[]; environment: Environment; urbanLevel: UrbanLevel; samplingMode: SamplingMode; panoramaSource: PanoramaSource; allowInteriors: boolean; showCompass: boolean }) => { uploadedHistoryRef.current = { locations: [], index: -1 }; setCanPreviousUploaded(false); skipUploadedSeedRef.current = !!ctx.studyImportedMapIdRef.current; const focused = settings.countryCodes || []; const collection = focused.length ? { id: `focus:${focused.join(',')}`, name: focused.map((code) => COUNTRIES[code]?.name || code).join(' + '), countryCodes: focused } : ctx.allCollections.find((item: Collection) => item.id === settings.collectionId) || activeCollectionRef.current; setTemporaryCollection(focused.length ? collection : null); setSelectedCollectionId(collection.id); activeCollectionRef.current = collection; setStudyEnvironment(settings.environment); setStudyUrbanLevel(settings.urbanLevel); setStudySampling(settings.samplingMode); setStudyPanoramaSource(settings.panoramaSource); setStudyAllowInteriors(settings.allowInteriors); studyEnvironmentRef.current = settings.environment; studyUrbanLevelRef.current = settings.urbanLevel; studySamplingRef.current = settings.samplingMode; studyPanoramaSourceRef.current = settings.panoramaSource; studyAllowInteriorsRef.current = settings.allowInteriors; setCompassPreference(settings.showCompass); setStudyReviewSaved(false); setIsStudySetupOpen(false); setShowHome(false); setAppMode('study'); setCurrentLocation(null); void Promise.all([trainerDb.setSetting('selectedCollectionId', settings.collectionId), trainerDb.setSetting('preference.environment', settings.environment), trainerDb.setSetting('preference.urbanLevel', settings.urbanLevel), trainerDb.setSetting('preference.sampling', settings.samplingMode), trainerDb.setSetting('preference.panoramaSource', settings.panoramaSource), trainerDb.setSetting('preference.allowContributors', settings.panoramaSource !== 'official'), trainerDb.setSetting('preference.allowInteriors', settings.allowInteriors), trainerDb.setSetting('preference.compass', settings.showCompass)]); if (mapsReady) setTimeout(() => void fetchNextLocation(), 0); };
  ...
  return { flushStudyActive, closeStudyVisit, handleStudyMetadata, handleStudyPanoramaChanged, fetchNextLocation, handlePreviousUploaded, canPreviousUploaded, handleMapsLoaded, handleSelectCollection, currentIsBookmarked, handleToggleBookmark, handleSelectBookmark, handleSaveStudyForReview, handleTrainCountries, handleStartStudy, handleOpenCoverageLocation, toggleFullscreen, activeCompass, toggleCompass };
}
