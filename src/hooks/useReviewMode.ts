...
import { reviewGradeForCorrection, reviewGradeForPerformance, trainerDb } from '../data/trainerDb';
...
export function useReviewMode(ctx: any) {
  ...
  const handleReviewNext = useCallback(async () => {
    ...
    try {
      const previous = (await trainerDb.reviews()).find((item) => item.panoId === reviewAttempt.panoId); const scheduled = reviewKind !== 'practice';
      ...
      const remaining = reviewQueue.slice(1); if (grade === 'again') remaining.splice(Math.min(1, remaining.length), 0, reviewAttempt); setReviewQueue(remaining); setTrainerRefreshKey((key: number) => key + 1);
