...
export function CoveragePanel({ locations, attempts, reviews, countries, unseen, clues, countryFilter, setCountryFilter, clueCountry, setClueCountry, load, onOpen }: CoveragePanelProps) {
  ...
  return <>
    ...
    <CoverageMap locations={locations.filter((item) => !countryFilter || item.countryCode === countryFilter)} attempts={attempts} reviews={reviews} onOpen={onOpen} />
    ...
    <div className="table-scroll"><table><thead><tr>{header("name", t("tableCountry"))}{header("seen", t("seen"))}{header("played", t("played"))}{header("reviewed", t("reviewedCount"))}{header("correct", t("correct"))}{header("wrong", t("wrong"))}{header("accuracy", t("accuracyTable"))}{header("average", t("averageScoreTable"))}{header("best", t("bestTable"))}{header("lastSeen", t("lastSeen"))}{header("clues", t("knownClues"))}</tr></thead><tbody>
      {rows.map((item) => <tr key={item.code} className={!item.seen ? "never-seen" : ""} onClick={() => setCountryFilter(item.code)}><th><CountryFlag code={item.code} />{item.name}</th><td>{item.seen || t("neverSeen")}</td><td>{item.played}</td><td>{item.reviewed}</td><td>{item.correct}</td><td>{item.wrong}</td><td>{Math.round(item.accuracy * 100)}%</td><td>{item.played ? Math.round(item.average).toLocaleString() : "—"}</td><td>{item.best || "—"}</td><td>{date(item.lastSeen)}</td><td>{item.clues ? <button className="table-link" onClick={(event) => { event.stopPropagation(); setClueCountry(item.code); }}>{item.clues}</button> : "—"}</td></tr>)}
    </tbody></table></div>
