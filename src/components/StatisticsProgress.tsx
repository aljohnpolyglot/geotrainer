...
function Matrix({ attempts, row, columns, column }: { attempts: Attempt[]; row: (item: Attempt) => string | undefined; columns: string[]; column: (item: Attempt) => string | undefined }) {
  ...
  const { ui } = useLanguagePreferences(); const t = (key: string) => translate(ui, key);
  return <div className="table-scroll"><table><thead><tr><th>{t('Group')}</th>{columns.map((item) => <th key={item}>{t(item)}</th>)}</tr></thead><tbody>{rows.map((key) => <tr key={key}><th>{key}</th>{columns.map((col) => { const value = metrics(attempts.filter((item) => row(item) === key && column(item) === col)).ladder.country; return <td key={col}>{pct(value.rate)}<small> n={value.eligible}</small></td>; })}</tr>)}</tbody></table></div>;
}
...
export function StatisticsProgress({ attempts, sessions, visits, reviews, collections, locations = [], includeAssisted = false }: { attempts: Attempt[]; sessions: TrainingSession[]; visits: StudyVisit[]; reviews: ReviewRecord[]; collections: Collection[]; locations?: import('../types').TrainerLocation[]; includeAssisted?: boolean }) {
  ...
  return <>
    ...
    <h3>{t('Country')} × {t('Environment')}</h3><Matrix attempts={attempts} row={(item) => name(item.countryCode)} columns={['urban', 'suburban', 'rural', 'mixed']} column={environmentOf} />
    ...
    <h3>{t('Country')} × {t('Movement')}</h3><Matrix attempts={attempts} row={(item) => name(item.countryCode)} columns={['Standard', 'No Move', 'NMPZ']} column={movementMode} />
