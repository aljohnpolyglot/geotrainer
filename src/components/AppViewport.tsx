...
import { ArrowLeft, Gamepad2, Globe2, History, Play } from 'lucide-react';
...
interface AppViewportProps {
  ...
  onStudy: () => void; onPlay: () => void; onReview: () => void;
...
export function AppViewport({
  ...
  useEffect(() => {
    setCachedStudyReveal((previous) => {
      if (!showHome && appMode === 'study' && isRevealed && currentLocation && !isLoading) return currentLocation;
      if (!showHome && appMode === 'study' && currentLocation?.panoId === previous?.panoId) return previous;
      return null;
    });
  ...
  return <main className="flex-1 w-full h-[calc(100dvh-3.5rem)] relative overflow-hidden bg-black">
    <StreetViewContainer currentLocation={currentLocation} isLoading={isLoading} onNextLocation={onNextLocation} statusMessage={statusMessage} errorMessage={errorMessage} onMapsLoaded={onMapsLoaded} canMove={canMove} canPan={canPan} canZoom={canZoom} showCompass={!showHome && activeCompass} compassStyle={compassStyle} restoredView={appMode === 'review' ? undefined : restoredStreetView} onViewChanged={onStreetViewChanged} onPanoramaChanged={!showHome && (appMode === 'study' || summaryRound) ? onPanoramaChanged : undefined} />
