...
import { breakdown, environmentOf, metrics, sampleLabel } from '../analytics/statistics';
...
export function CountryStatistics({ play, attempts, visits, locations, reviews }: { play: Attempt[]; attempts: Attempt[]; visits: StudyVisit[]; locations: TrainerLocation[]; reviews: ReviewRecord[] }) {
  ...
  return <>
    ...
    <div className="table-scroll"><table><thead><tr>{header('name', t('Country'))}{header('seen', t('panoramas'))}{header('study', t('Study'))}{header('play', t('Play'))}{header('review', t('Review'))}{header('correct', t('Correct'))}{header('country', t('Country'))}{header('region', t('Region'))}{header('score', t('Avg / median / best'))}{header('distance', t('Avg / median km'))}{header('time', t('Avg time'))}{header('last', t('Last seen'))}{header('due', t('Due'))}{header('sample', t('Sample'))}</tr></thead><tbody>{sorted.map((row) => <tr key={row.code} onClick={() => setSelected(row.code)} className={selected === row.code ? 'selected' : ''}><th><CountryFlag code={row.code} />{name(row.code)}</th><td>{row.seen.length}</td><td>{row.exposure.length}</td><td>{row.played.length}</td><td>{row.reviewAttempts.length}</td><td>{row.value.ladder.country.correct}</td><td>{pct(row.value.ladder.country.rate)} <small>n={row.value.ladder.country.eligible}</small></td><td>{pct(row.value.ladder.region.rate)}</td><td>{num(row.value.averageScore)} / {num(row.value.medianScore)} / {Math.max(0, ...row.played.map((item) => item.score))}</td><td>{num(row.value.averageDistance, 1)} / {num(row.value.medianDistance, 1)}</td><td>{num(row.value.averageTime, 1)}s</td><td>{row.lastSeen ? new Date(row.lastSeen).toLocaleDateString() : '—'}</td><td>{row.due}</td><td><span className={`sample ${row.played.length < 15 ? 'low' : ''}`}>{t(sampleLabel(row.played.length))}</span></td></tr>)}</tbody></table></div>
    {current && <section className="country-detail stats-card"><h3><CountryFlag code={current.code} />{name(current.code)} · {t('Details')}</h3><div className="metric-strip"><div><span>{t('Country')}</span><strong>{pct(current.value.ladder.country.rate)}</strong></div><div><span>{t('Region')}</span><strong>{pct(current.value.ladder.region.rate)}</strong></div><div><span>{t('Avg score')}</span><strong>{num(current.value.averageScore)}</strong></div><div><span>{t('Recent 10')}</span><strong>{num(metrics(current.played.slice(-10)).averageScore)}</strong></div><div><span>{t('Avg time')}</span><strong>{num(current.value.averageTime, 1)}s</strong></div></div>
      <div className="stats-ladders"><section><h3>{t('When actual')} = <CountryFlag code={current.code} />{name(current.code)}</h3>{counts(actualMistakes, (item) => item.guessedCountryCode).slice(0, 10).map(([code, count]) => <div className="data-row" key={code}><span><CountryFlag code={code} />{name(code)}</span><b>{count}</b></div>)}</section><section><h3>{t('Mistaken for')} <CountryFlag code={current.code} />{name(current.code)}</h3>{counts(mistakenFor, (item) => item.countryCode).slice(0, 10).map(([code, count]) => <div className="data-row" key={code}><span><CountryFlag code={code} />{name(code)}</span><b>{count}</b></div>)}</section></div>
      <h3>{t('Environment')}</h3><Simple rows={breakdown(current.played, environmentOf)} t={t} /><h3>{t('Movement')}</h3><Simple rows={breakdown(current.played, (item) => item.canMove ? 'Standard' : item.canPan || item.canZoom ? 'No Move' : 'NMPZ')} t={t} />
      <h3>{t('Recent attempts and reviews')}</h3><div className="attempt-list">{currentAll.slice().sort((a, b) => b.createdAt - a.createdAt).slice(0, 20).map((item) => <div className="attempt-row" key={item.id}><span>{new Date(item.createdAt).toLocaleDateString()} · {t(item.source)}</span><span>{item.guessedCountryCode ? <><CountryFlag code={item.guessedCountryCode} />{name(item.guessedCountryCode)}</> : t('Country unresolved')}</span><strong>{item.score}</strong></div>)}</div>
    </section>}
  </>;
}
