...
export function TrainerHubClues({ clues, learnedMetas, notebookNotes, coachNotes, onDelete, onDeleteNote, onTrainCountries }: { clues: ClueRecord[]; learnedMetas: LearnedMeta[]; notebookNotes: NotebookNote[]; coachNotes: CoachHistoryNote[]; locations: TrainerLocation[]; onDelete: (id: string) => void; onDeleteNote: (note: NotebookNote) => void; onTrainCountries: (codes: string[], name: string) => void }) {
...
<div className="clue-row-actions"><button className="icon-button" onClick={(event) => { event.stopPropagation(); onDeleteNote(note); }} aria-label={t('Delete note')} title={t('Delete note')}><Trash2 size={16} /></button></div>
