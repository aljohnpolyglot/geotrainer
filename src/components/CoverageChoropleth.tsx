...
const colorMap = (values: Record<string, number | null>, colors: readonly string[], label: string, emptyLabel: string, details: Record<string, string>) => worldMap
  ...
  .replace(/<path\b([^>]*\bid="([A-Z]{2})"[^>]*)\/>/g, (_match, attributes: string, id: string) => {
    ...
    return `<path${attributes.replace(/\sstyle="[^"]*"/, '')} style="fill:${fill};outline:none" role="button" tabindex="0" aria-label="${escapeXml(countryName(id))}"><title>${escapeXml(countryName(id) + (detail ? ` · ${detail}` : ''))}</title></path>`;
  });
...
export function CoverageChoropleth({ locations, attempts, reviews, overlay }: { locations: TrainerLocation[]; attempts: Attempt[]; reviews: ReviewRecord[]; overlay: CoverageOverlay }) {
  ...
  const values = useMemo(() => coverageCountryValues(locations, attempts, reviews, overlay, Date.now(), overlay === 'exposure' ? 250 : overlay === 'due' ? 25 : 1), [locations, attempts, reviews, overlay]);
  ...
  return <section className="coverage-choropleth" style={borderStyle}>
    ...
    {selectedCountry && <CoverageRegionModal countryCode={selectedCountry} locations={locations} attempts={attempts} reviews={reviews} overlay={overlay} colors={colors} onClose={() => setSelectedCountry(undefined)} />}
  </section>;
}
