...
import type { Attempt, ClueRecord, CoachHistoryNote, Collection, GameRecord, LearnedMeta, NotebookNote, ReviewFilters, ReviewRecord, ReviewSessionKind, SchedulerPreferences, StudyVisit, TrainerLocation } from "../types";
...
export function TrainerHub({ collections, refreshKey, onReview, onOpen, onTrainCountries, onSelectGame, initialTab = "review", initialHistoryKind = "games", initialStatisticsSection = "overview", onViewStateChange }: TrainerHubProps) {
  ...
  const [coachNotes, setCoachNotes] = useState<CoachHistoryNote[]>([]);
  ...
  const [readyDueCount, setReadyDueCount] = useState(0);
  ...
  const [visitedStatistics, setVisitedStatistics] = useState(initialTab === "statistics");
  const [visitedCoverage, setVisitedCoverage] = useState(initialTab === "coverage");
  ...
  useEffect(() => {
    if (tab === "statistics") setVisitedStatistics(true);
    if (tab === "coverage") setVisitedCoverage(true);
  }, [tab]);
  useEffect(() => {
  ...
  const mistakeCriterion = [filters.wrongCountry && t("wrongCountry"), filters.minScore !== undefined || filters.maxScore !== undefined ? `${t("roundScore")} ${filters.minScore ?? 0}–${filters.maxScore ?? 5000}` : ''].filter(Boolean).join(" · ");
  const reviewCriterion = [filters.due && t("dueToday"), mistakeCriterion, filters.recent && t("recentlyMissed"), filters.recentDays && ([7, 30, 90].includes(filters.recentDays) ? t(`last${filters.recentDays}Days`) : `${t("lastDays")} ${filters.recentDays}`), filters.bookmarked && t("bookmarked")].filter(Boolean).join(" · ") || t("tabReview");
  const reviewSource = [filters.environment && t(filters.environment), reviewCriterion].filter(Boolean).join(" · ");
  ...
  const historyPanel = <HistoryPanel historyKind={historyKind} setHistoryKind={(next) => persistView({ historyKind: next })} setCountryFilter={setCountryFilter} countryFilter={countryFilter} collectionFilter={collectionFilter} setCollectionFilter={setCollectionFilter} dateFilter={dateFilter} setDateFilter={setDateFilter} minScore={minScore} setMinScore={setMinScore} maxDistance={maxDistance} setMaxDistance={setMaxDistance} correctness={correctness} setCorrectness={setCorrectness} sourceFilter={sourceFilter} setSourceFilter={setSourceFilter} collections={collections} countries={countries} games={games} filteredAttempts={filteredAttempts} visits={visits} clues={clues} notebookNotes={notebookNotes} reviews={reviews} locations={locations} onOpen={onOpen} startReview={startReview} onSelectGame={onSelectGame} />;
  return <section className="trainer-hub">
  <header className="hub-header"><div><h1>{tab === "statistics" ? t("tabStatistics") : tab === "clues" ? t("Clues") : t("tabReview")}</h1><p>{tab === "statistics" ? t("tagline") : tab === "clues" ? t("Your saved visual clues, organized for study.") : t("reviewsReadyDescription")}</p></div></header>
    {!loaded ? <div className="hub-loading"><RefreshCw className="spin" /> {t("loading")}</div> : <div className="hub-content">
      ...
      {visitedStatistics && <div hidden={tab !== "statistics"}><StatisticsPanel attempts={attempts} visits={visits} locations={locations} reviews={reviews} readyDueCount={readyDueCount} sessions={sessions} collections={collections} onTrainCountries={onTrainCountries} initialSection={statisticsSection} onSectionChange={(next) => persistView({ statisticsSection: next })} locationsPanel={<CoveragePanel locations={locations} attempts={attempts} reviews={reviews} countries={countries} unseen={unseen} clues={clues} countryFilter={countryFilter} setCountryFilter={setCountryFilter} clueCountry={clueCountry} setClueCountry={setClueCountry} load={load} onOpen={onOpen} />} historyPanel={historyPanel} /></div>}
      ...
      {visitedCoverage && <div hidden={tab !== "coverage"}><CoveragePanel locations={locations} attempts={attempts} reviews={reviews} countries={countries} unseen={unseen} clues={clues} countryFilter={countryFilter} setCountryFilter={setCountryFilter} clueCountry={clueCountry} setClueCountry={setClueCountry} load={load} onOpen={onOpen} /></div>}
