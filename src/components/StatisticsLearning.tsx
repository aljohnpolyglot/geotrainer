...
export function ReviewStatistics({ attempts, reviews }: { attempts: Attempt[]; reviews: ReviewRecord[] }) {
  ...
  return <>
    ...
    <div className="stats-ladders"><section className="stats-card"><h3>{t('Lapsed countries')}</h3>{learning.lapseCountries.slice(0, 12).map((item) => <div className="data-row" key={item.key}><span>{name(item.key)}</span><b>{item.count}</b></div>)}{!learning.lapseCountries.length && <p className="empty">{t('No learned-to-wrong review transitions yet.')}</p>}</section><section className="stats-card"><h3>{t('Lapsed locations')}</h3>{learning.lapseLocations.slice(0, 12).map((item) => <div className="data-row" key={item.key}><span>{item.key.slice(0, 14)}…</span><b>{item.count}</b></div>)}{!learning.lapseLocations.length && <p className="empty">{t('No location lapses yet.')}</p>}</section></div>
  </>;
}
...
export function ConfusionStatistics({ attempts, onTrainCountries }: { attempts: Attempt[]; onTrainCountries: (codes: string[], name: string) => void }) {
  ...
  const list = (title: string, items: typeof country.directional, symbol: string) => <section className="stats-card"><h3>{t(title)}</h3>{items.slice(0, 25).map((item) => <button className="data-row" key={item.codes.join(':')} onClick={() => onTrainCountries(item.codes, t('Confusion drill'))}><span>{name(item.codes[0])} {symbol} {name(item.codes[1])}</span><b>{item.count}</b></button>)}{!items.length && <p className="empty">{t('No resolved country confusions yet.')}</p>}</section>;
