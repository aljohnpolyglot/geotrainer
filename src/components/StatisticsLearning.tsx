...
export function ReviewStatistics({ attempts, reviews }: { attempts: Attempt[]; reviews: ReviewRecord[] }) {
  ...
  const attemptByPano = new Map(attempts.map((item) => [item.panoId, item]));
  return <>
    ...
    <div className="stats-ladders"><section className="stats-card"><h3>{t('Lapsed countries')}</h3>{learning.lapseCountries.slice(0, 12).map((item) => <div className="data-row" key={item.key}><span><CountryFlag code={item.key} />{name(item.key)}</span><b>{item.count}</b></div>)}{!learning.lapseCountries.length && <p className="empty">{t('No learned-to-wrong review transitions yet.')}</p>}</section><section className="stats-card"><h3>{t('Lapsed locations')}</h3>{learning.lapseLocations.slice(0, 12).map((item) => { const attempt = attemptByPano.get(item.key); return <div className="data-row" key={item.key}><span>{attempt ? <><CountryFlag code={attempt.countryCode} />{name(attempt.countryCode)}</> : t('Unknown location')}</span><b>{item.count}</b><a className="icon-button" href={`https://www.google.com/maps/@?api=1&map_action=pano&pano=${encodeURIComponent(item.key)}`} target="_blank" rel="noreferrer" aria-label={t('openLocation')}><ArrowUpRight size={15} /></a></div>; })}{!learning.lapseLocations.length && <p className="empty">{t('No location lapses yet.')}</p>}</section></div>
  </>;
}
...
export function ConfusionStatistics({ attempts, onTrainCountries }: { attempts: Attempt[]; onTrainCountries: (codes: string[], name: string) => void }) {
  const { ui } = useLanguagePreferences(); const t = (key: string) => translate(ui, key);
  const [directionalPage, setDirectionalPage] = useState(1); const [symmetricPage, setSymmetricPage] = useState(1); const [regionPage, setRegionPage] = useState(1);
  const country = confusions(attempts); const regions = regionConfusions(attempts);
  const pager = (paging: ReturnType<typeof pageBounds>, setPage: (page: number) => void, label: string) => paging.pages > 1 && <nav className="clue-pagination" aria-label={label}><button type="button" disabled={paging.current === 1} onClick={() => setPage(paging.current - 1)}>{t('Previous')}</button><span aria-live="polite">{paging.current} / {paging.pages}</span><button type="button" disabled={paging.current === paging.pages} onClick={() => setPage(paging.current + 1)}>{t('Next')}</button></nav>;
  const list = (title: string, items: typeof country.directional, symbol: string, page: number, setPage: (page: number) => void) => { const paging = pageBounds(items.length, page, 10); return <section className="stats-card"><h3>{t(title)}</h3>{items.slice(paging.start, paging.end).map((item) => <button className="data-row" key={item.codes.join(':')} onClick={() => onTrainCountries(item.codes, t('Confusion drill'))}><span><CountryFlag code={item.codes[0]} />{name(item.codes[0])} {symbol} <CountryFlag code={item.codes[1]} />{name(item.codes[1])}</span><b>{item.count}</b></button>)}{!items.length && <p className="empty">{t('No resolved country confusions yet.')}</p>}{pager(paging, setPage, t(title))}</section>; };
  const regionPaging = pageBounds(regions.length, regionPage, 10);
  return <><div className="stats-ladders">{list('Directional', country.directional, '→', directionalPage, setDirectionalPage)}{list('Symmetric', country.symmetric, '↔', symmetricPage, setSymmetricPage)}</div><section className="stats-card"><h3>{t('Region confusions')}</h3>{regions.slice(regionPaging.start, regionPaging.end).map((item) => <div className="data-row" key={item.pair}><span>{item.pair}</span><b>{item.count}</b></div>)}{!regions.length && <p className="empty">{t('No resolved region confusions yet.')}</p>}{pager(regionPaging, setRegionPage, t('Region confusions'))}</section></>;
}
