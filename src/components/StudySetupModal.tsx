...
export function StudySetupModal({ open, collections, initial, onClose, onStart }: { open: boolean; collections: Collection[]; initial: StudySetup; onClose: () => void; onStart: (settings: StudySetup) => void }) {
  ...
  return <div className="setup-backdrop"><section className="setup-dialog" aria-labelledby="study-setup-title">
    ...
    <div className="setup-body">
      <label>{t('Map Collection')}<select value={settings.collectionId} onChange={(event) => setSettings({ ...settings, collectionId: event.target.value, countryCodes: undefined })}>{BUILT_IN_COLLECTION_GROUPS.map((group) => <optgroup label={group.label} key={group.label}>{group.collections.map((item) => <option key={item.id} value={item.id}>{item.name} ({item.countryCodes.length})</option>)}</optgroup>)}{collections.some((item) => item.isCustom) && <optgroup label={t('Custom collections')}>{collections.filter((item) => item.isCustom).map((item) => <option key={item.id} value={item.id}>{item.name} ({item.countryCodes.length})</option>)}</optgroup>}</select></label>
