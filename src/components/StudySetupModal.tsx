...
import { META_LESSONS, savedMetaLessonIds } from '../data/metaLessons';
...
export function StudySetupModal({ open, collections, initial, onClose, onStart }: { open: boolean; collections: Collection[]; initial: StudySetup; onClose: () => void; onStart: (settings: StudySetup) => void }) {
  ...
  const [metaView, setMetaView] = useState<MetaView>('general');
  ...
  useEffect(() => { if (!open) return; let active = true; setSettings(normalizeStudySetup(initial, collections)); setPanel(initial.source); setMetaView('general'); setMetaAvailable(null); void trainerDb.attempts().then((attempts) => { if (!active) return; const completed = savedMetaLessonIds(attempts); const available = completed.size < META_LESSONS.length; setCompletedMetaIds(completed); setMetaAvailable(available); if (!available) setPanel((value) => value === 'meta' ? 'custom' : value); }); return () => { active = false; }; }, [open]);
  if (!open) return null;
  ...
  return <div className="setup-backdrop"><section className="setup-dialog" aria-labelledby="study-setup-title">
    ...
    <div className="setup-body">
      ...
      {panel === 'meta' && <div className="meta-setup">
        <div className="meta-setup-tabs" role="tablist" aria-label={t('Meta lesson mode')}><button type="button" role="tab" aria-selected={metaView === 'general'} className={metaView === 'general' ? 'selected' : ''} onClick={() => { setMetaView('general'); setSettings({ ...settings, metaLessonId: undefined }); }}>{t('General')}</button><button type="button" role="tab" aria-selected={metaView === 'browse'} className={metaView === 'browse' ? 'selected' : ''} onClick={() => setMetaView('browse')}>{t('Browse')}</button></div>
        {metaView === 'general' ? <div className="meta-general"><Lightbulb size={20} aria-hidden="true" /><span><strong>{t('General Meta')}</strong><small>{t('Continue with a random unfinished lesson.')}</small></span><b aria-label={`${META_LESSONS.length - completedMetaIds.size} ${t('lessons remaining')}`}>{META_LESSONS.length - completedMetaIds.size}</b></div> : <MetaLessonBrowser completed={completedMetaIds} selectedId={settings.metaLessonId} onSelect={(metaLessonId) => setSettings({ ...settings, metaLessonId })} />}
      </div>}
    ...
    <footer><button className="button primary" disabled={(panel === 'meta' && (metaAvailable !== true || (metaView === 'browse' && !settings.metaLessonId))) || ((panel === 'uploaded' || panel === 'url') && !importedMap) || panel === 'describe'} onClick={() => onStart({ ...settings, source: panel === 'url' ? 'uploaded' : panel as LearnSource, importedMapId: importedMap?.id, showCompass: true })}><Play size={16} />{t(panel === 'meta' && metaView === 'browse' ? 'Start selected lesson' : panel === 'meta' ? 'Start Meta lessons' : panel === 'map' ? 'Open world map' : 'Start learning')}</button></footer>
  </section></div>;
}
