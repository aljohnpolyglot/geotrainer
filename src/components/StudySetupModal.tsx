...
import type { Collection, Environment, LearnSource, SamplingMode, UrbanLevel } from '../types';
import { CountryMixPicker } from './CountryMixPicker';
import { CollectionOptions } from './CollectionOptions';
import { translate } from '../services/language';
import { useLanguagePreferences } from '../services/useLanguagePreferences';

export type StudySetup = { source: LearnSource; collectionId: string; countryCodes?: string[]; environment: Environment; urbanLevel: UrbanLevel; samplingMode: SamplingMode; allowContributors: boolean; showCompass: boolean };
  ...
  return <div className="setup-backdrop"><section className="setup-dialog" aria-labelledby="study-setup-title">
    ...
    <div className="setup-body">
      ...
      {settings.source === 'custom' && <div className="learn-custom-settings"><label>{t('Map Collection')}<select value={settings.collectionId} onChange={(event) => setSettings({ ...settings, collectionId: event.target.value, countryCodes: undefined })}><CollectionOptions collections={collections} customLabel={t('Custom collections')} /></select></label>
        ...
        <button type="button" role="switch" aria-checked={settings.allowContributors} className={`learn-contributor-toggle ${settings.allowContributors ? 'enabled' : ''}`} onClick={() => setSettings({ ...settings, allowContributors: !settings.allowContributors })}><span>{t('Contributor panoramas')}</span><strong>{t(settings.allowContributors ? 'Included' : 'Official only')}</strong></button>
      </div>}
    </div>
