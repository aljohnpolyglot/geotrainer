...
export function StudySetupModal({ open, collections, initial, onClose, onStart }: { open: boolean; collections: Collection[]; initial: StudySetup; onClose: () => void; onStart: (settings: StudySetup) => void }) {
  ...
  return <div className="setup-backdrop"><section className="setup-dialog" aria-labelledby="study-setup-title">
    ...
    <div className="setup-body">
      ...
      {settings.source === 'custom' && <div className="learn-custom-settings"><label>{t('Map Collection')}<select value={settings.collectionId} onChange={(event) => setSettings({ ...settings, collectionId: event.target.value, countryCodes: undefined, locationTargets: undefined })}><CollectionOptions collections={collections} customLabel={t('Custom collections')} /></select></label>
        ...
        {!!settings.countryCodes?.length && <div className="setup-field"><label>{t('Location pools')}</label><LocationPoolPicker countryCodes={settings.countryCodes} value={settings.locationTargets || []} onChange={(locationTargets) => setSettings({ ...settings, locationTargets: locationTargets.length ? locationTargets : undefined })} /></div>}
