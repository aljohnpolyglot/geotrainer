...
import { loadCityPools, localizedPoolName, locationPoolChoices, locationTargetKey, samePoolRegion, selectPoolCity, selectPoolRegion } from '../services/cityPools';
...
export function LocationPoolPicker({ countryCodes, value, onChange }: { countryCodes: string[]; value: LocationPoolTarget[]; onChange: (targets: LocationPoolTarget[]) => void }) {
  ...
  const countryName = (code: string) => countryDisplayName(code, ui) || COUNTRIES[code]?.name || code;
  ...
  const choices = useMemo(() => locationPoolChoices(countryCodes, pools, value, order, ui), [countryCodes, order, pools, ui, value]);
  ...
  const regionFor = (target: LocationPoolTarget) => pools[target.countryCode]?.find((region) => region.id === target.regionId);
  const regionName = (target: LocationPoolTarget) => { const region = regionFor(target); return region ? localizedPoolName(region, ui) : target.regionName; };
  const cityName = (target: Extract<LocationPoolTarget, { kind: 'city' }>) => localizedPoolName(regionFor(target)?.cities.find((city) => city.lat === target.city.lat && city.lng === target.city.lng) || target.city, ui);
  ...
  return <div className="location-pool-picker" aria-busy={loading}>
    <div className="location-pool-field">
      <label>{t('Regions')}<select disabled={loading} value="" onChange={(event) => { const target = byKey.get(event.target.value); if (target) onChange(selectPoolRegion(value, target)); }}>
        ...
        {choices.map(({ countryCode, regions: available }) => <optgroup key={countryCode} label={countryName(countryCode)}>
          {available.flatMap((region) => region.targets.filter((target) => target.kind === 'region' && !regions.some((item) => samePoolRegion(item, target))).map((target) => <option key={locationTargetKey(target)} value={locationTargetKey(target)}>{localizedPoolName(region, ui)}</option>))}
        </optgroup>)}
      </select></label>
      {!!regions.length && <div className="country-mix-chips">{regions.map((target) => <button type="button" key={`${target.countryCode}:${target.regionId}`} onClick={() => onChange(value.filter((item) => !samePoolRegion(item, target)))} aria-label={`${t('Remove')} ${regionName(target)}`}><CountryFlag code={target.countryCode} /><span>{regionName(target)}</span><X size={13} /></button>)}</div>}
    </div>
    {!!regions.length && <div className="location-pool-field">
      <div className="location-pool-controls"><label>{t('Cities')}<select disabled={loading} value="" onChange={(event) => { const target = byKey.get(event.target.value); if (target) onChange(selectPoolCity(value, target)); }}>
        ...
        {choices.flatMap(({ countryCode, regions: available }) => available.filter((region) => regions.some((item) => item.countryCode === countryCode && item.regionId === region.id)).map((region) => <optgroup key={`${countryCode}:${region.id}`} label={`${countryName(countryCode)} — ${localizedPoolName(region, ui)}`}>
          {region.targets.filter((target) => target.kind === 'city').map((target) => <option key={locationTargetKey(target)} value={locationTargetKey(target)}>{target.kind === 'city' && localizedPoolName(target.city, ui)}</option>)}
        </optgroup>))}
      ...
      {!!cities.length && <div className="country-mix-chips">{cities.map((target) => <button type="button" key={locationTargetKey(target)} onClick={() => onChange(selectPoolCity(value, target, true))} aria-label={`${t('Remove')} ${cityName(target)}`} title={regionName(target)}><CountryFlag code={target.countryCode} /><span>{cityName(target)}</span><X size={13} /></button>)}</div>}
    </div>}
  </div>;
}
