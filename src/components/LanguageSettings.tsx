...
import { announceMapPreferences, countryBorderColor, countryBorderWeight, DEFAULT_MAP_PREFERENCES, normalizeMapPreferences } from '../services/mapPreferences';
...
export function LanguageSettings({ open, onClose, onChange }: { open: boolean; onClose: () => void; onChange?: (value: LanguagePreferences, compassStyle: CompassStyle, darkMode: boolean) => void }) {
  ...
  const previewDark = mapPreferences.mapPalette === 'dark' || mapPreferences.mapPalette === 'auto' && darkMode; const previewBorder = countryBorderColor(mapPreferences.countryBorderColor, previewDark); const previewWidth = countryBorderWeight(mapPreferences.countryBorderWidth);
  ...
  return <div className="language-settings-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="language-settings preferences-modal" role="dialog" aria-modal="true" aria-labelledby="preferences-title">
      ...
      {tab === 'display' && <><fieldset><legend>{translate(languages.ui, 'Appearance')}</legend>
        ...
        <div className={`map-border-preview${previewDark ? ' dark-map' : ''}`} style={{ '--country-border': mapPreferences.showCountryBorders === false ? 'transparent' : previewBorder, '--country-width': `${previewWidth * 2}px`, '--region-border': mapPreferences.showRegionBorders === true ? previewBorder : 'transparent', '--region-width': `${Math.max(.2, previewWidth * 1.2)}px` } as CSSProperties}><span>{translate(languages.ui, 'Map preview')}</span><svg viewBox="0 0 360 116" role="img" aria-label={translate(languages.ui, 'Map preview')}><rect width="360" height="116" /><g className="preview-land"><path d="M-12 18L61 8l35 18 50-8 38 25-18 63H35L8 79Z"/><path d="M196 7l76 8 25 19 75-4v86H183l-8-38 24-24Z"/></g><g className="preview-regions"><path d="M61 8l5 41-58 30M96 26l-22 39 92 41M184 43l-62 20M272 15l-18 45-71 18M297 34l-43 26 23 56"/></g><g className="preview-roads"><path d="M8 92C74 59 115 72 183 49S289 26 372 50"/><path d="M43 6c51 52 116 39 152 97"/><path d="M221 9c8 45 43 65 101 96"/></g><g className="preview-labels"><text x="48" y="39">Norland</text><text x="216" y="43">Eastland</text><text x="126" y="91">River City</text></g></svg></div>
      </div></fieldset></>}
