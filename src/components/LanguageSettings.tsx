...
const AUTO_REVIEW_COPY: Record<SupportedLanguage, { title: string; description: string }> = {
  en: { title: 'Add Play rounds to Review', description: 'Adds a card when either condition matches.' }, es: { title: 'Añadir rondas de juego al repaso', description: 'Añade una tarjeta si se cumple cualquiera de las condiciones.' }, pt: { title: 'Adicionar rodadas de jogo à revisão', description: 'Adiciona um cartão se qualquer condição corresponder.' }, fr: { title: 'Ajouter les manches de jeu à la révision', description: 'Ajoute une carte si l’une des conditions est remplie.' },
  de: { title: 'Spielrunden zur Wiederholung hinzufügen', description: 'Fügt eine Karte hinzu, wenn eine der Bedingungen zutrifft.' }, it: { title: 'Aggiungi turni di gioco al ripasso', description: 'Aggiunge una carta se una delle condizioni corrisponde.' }, ru: { title: 'Добавлять игровые раунды в повторение', description: 'Добавляет карточку, если выполняется любое условие.' }, sv: { title: 'Lägg till spelrundor för repetition', description: 'Lägger till ett kort när något av villkoren matchar.' },
};
...
export function LanguageSettings({ open, onClose, onChange }: { open: boolean; onClose: () => void; onChange?: (value: LanguagePreferences, compassStyle: CompassStyle, darkMode: boolean) => void }) {
  ...
  const previewDark = mapPreferences.mapPalette === 'dark' || mapPreferences.mapPalette === 'auto' && darkMode;
  ...
  return <div className="language-settings-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="language-settings preferences-modal" role="dialog" aria-modal="true" aria-labelledby="preferences-title">
      ...
      {tab === 'display' && <><fieldset><legend>{translate(languages.ui, 'Appearance')}</legend>
        ...
        <label>{translate(languages.ui, 'Result map zoom')}<select value={mapPreferences.resultMapZoom ?? 'country'} onChange={(event) => setMapPreferences({ ...mapPreferences, resultMapZoom: event.target.value as MapPreferences['resultMapZoom'] })}><option value="closest">{translate(languages.ui, 'Closest')}</option><option value="country">{translate(languages.ui, 'Country')}</option><option value="region">{translate(languages.ui, 'Country region')}</option><option value="world">{translate(languages.ui, 'World')}</option></select></label>
        <label>{translate(languages.ui, 'Map color palette')}<select value={mapPreferences.mapPalette ?? 'auto'} onChange={(event) => setMapPreferences({ ...mapPreferences, mapPalette: event.target.value as MapPreferences['mapPalette'] })}><option value="auto">{translate(languages.ui, 'Automatic')}</option><option value="light">{translate(languages.ui, 'Light')}</option><option value="dark">{translate(languages.ui, 'Dark')}</option></select></label>
        ...
        <label className="settings-checkbox settings-detail"><input type="checkbox" checked={mapPreferences.showRegionBorders === true} onChange={(event) => setMapPreferences({ ...mapPreferences, showRegionBorders: event.target.checked })} /><span><strong>{translate(languages.ui, 'Regional borders')}</strong></span></label>
        <label>{translate(languages.ui, 'Country border width')}<select value={mapPreferences.countryBorderWidth ?? 'thin'} onChange={(event) => setMapPreferences({ ...mapPreferences, countryBorderWidth: event.target.value as MapPreferences['countryBorderWidth'] })}><option value="thin">{translate(languages.ui, 'Thin')}</option><option value="standard">{translate(languages.ui, 'Standard')}</option><option value="bold">{translate(languages.ui, 'Bold')}</option></select></label>
        <label>{translate(languages.ui, 'Country border color')}<select value={mapPreferences.countryBorderColor ?? 'auto'} onChange={(event) => setMapPreferences({ ...mapPreferences, countryBorderColor: event.target.value as MapPreferences['countryBorderColor'] })}><option value="auto">{translate(languages.ui, 'Automatic')}</option><option value="light">{translate(languages.ui, 'Light')}</option><option value="dark">{translate(languages.ui, 'Dark')}</option><option value="accent">{translate(languages.ui, 'Accent')}</option></select></label>
        <MapBorderPreview preferences={mapPreferences} dark={previewDark} label={translate(languages.ui, 'Map preview')} />
      </div></fieldset></>}
      ...
      <fieldset className="scheduling-settings"><legend>{AUTO_REVIEW_COPY[languages.ui].title}</legend>
        <label>{translate(languages.ui, 'scoreBelow')} {Math.round(scheduler.autoReviewScore)}<input type="range" min="0" max="5000" step="100" value={scheduler.autoReviewScore} onChange={(event) => setScheduler((current) => ({ ...current, autoReviewScore: Number(event.target.value) }))} /></label>
        <label className="settings-checkbox"><input type="checkbox" checked={scheduler.autoReviewWrongCountry} onChange={(event) => setScheduler((current) => ({ ...current, autoReviewWrongCountry: event.target.checked }))} />{translate(languages.ui, 'wrongCountry')}</label><p>{AUTO_REVIEW_COPY[languages.ui].description}</p>
      </fieldset>
