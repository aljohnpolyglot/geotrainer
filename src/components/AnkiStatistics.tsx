...
export function AnkiStatistics({ attempts, reviews, visits, locations }: Props) {
  ...
  return <div className="anki-statistics" aria-label={t('Anki statistics')}>
    ...
    <section className="stats-card anki-card"><h3>{t('Future due')}</h3><p className="stats-note">{t('Cards scheduled from today through the next 30 days.')}</p><div className="anki-bars" aria-label={t('Future due review histogram')}>{selectedDue && <output className="anki-bar-readout" aria-live="polite">{dayLabel(selectedDue.day)}: {selectedDue.count} {t('due')}</output>}{stats.futureDue.map((item, index) => <button type="button" className="anki-bar-column" key={item.day} title={`${dayLabel(item.day)}: ${item.count} ${t('due')}`} aria-label={`${dayLabel(item.day)}: ${item.count} ${t('due')}`} onClick={() => setSelectedDue(item)} onFocus={() => setSelectedDue(item)}><i style={{ height: `${item.count / futureMax * 100}%` }} />{(index === 0 || index % 5 === 0) && <small>{dayLabel(item.day)}</small>}</button>)}</div></section>
