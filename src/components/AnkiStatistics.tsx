...
export function AnkiStatistics({ attempts, reviews, visits, locations }: Props) {
  ...
  const [heldDue, setHeldDue] = useState<{ day: number; count: number } | null>(null);
  const holdTimer = useRef<ReturnType<typeof setTimeout>>();
  const endDueHold = () => { clearTimeout(holdTimer.current); holdTimer.current = undefined; setHeldDue(null); };
  const startDueHold = (item: { day: number; count: number }) => { clearTimeout(holdTimer.current); holdTimer.current = setTimeout(() => setHeldDue(item), 450); };
  ...
  return <div className="anki-statistics" aria-label={t('Anki statistics')}>
    ...
    <section className="stats-card anki-card"><h3>{t('Future due')}</h3><p className="stats-note">{t('Cards scheduled from today through the next 30 days.')}</p><div className="anki-bars" aria-label={t('Future due review histogram')}>{heldDue && <output className="anki-bar-readout" aria-live="polite">{dayLabel(heldDue.day)}: {heldDue.count} {t('due')}</output>}{stats.futureDue.map((item, index) => <button type="button" className="anki-bar-column" key={item.day} title={`${dayLabel(item.day)}: ${item.count} ${t('due')}`} aria-label={`${dayLabel(item.day)}: ${item.count} ${t('due')}`} onPointerDown={(event) => { if (event.pointerType !== 'mouse') startDueHold(item); }} onPointerUp={endDueHold} onPointerCancel={endDueHold} onPointerLeave={endDueHold} onContextMenu={(event) => event.preventDefault()} onFocus={() => setHeldDue(item)} onBlur={endDueHold}><i style={{ height: `${item.count / futureMax * 100}%` }} />{(index === 0 || index % 5 === 0) && <small>{dayLabel(item.day)}</small>}</button>)}</div></section>
