...
export function LearningAids({ lesson, panoId, countryCode, reviewActive, answerVisible, adviceOpen, refreshKey, onAdviceClose, onSaveClue, onNoteSaved }: {
  ...
  useEffect(() => {
    ...
    if (fullContent) void trainerDb.setting<NoteDraft>('workspace.noteDraft').then((draft) => { if (draft?.panoId === panoId) { setNote(draft.text); setCategory(draft.category); } });
  }, [fullContent, panoId]);
  ...
  return <>
    ...
    {open === 'notebook' && <aside ref={panelRef} style={dragStyle} className="learning-aid-panel notebook-panel" aria-label={t('Notebook')}>
      ...
      {fullContent ? <><ClueCapture panoId={panoId} onSave={(clue) => onSaveClue({ ...clue, origin: 'personal' })} onSaved={setNoteClueId} onImageChange={setNoteImage} spoilerFree={false} expanded collapseSavedAnalysis />
        ...
        <button className="notebook-save" disabled={noteSaved} onClick={() => void saveNotebookNote()}>{t(noteSaved ? 'Saved' : 'Save note for Review')}</button></> : <p className="learning-aid-safe">{t('Your personal notes appear after your guess.')}</p>}
    </aside>}
  </>;
}
