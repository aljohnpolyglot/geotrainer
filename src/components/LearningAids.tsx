...
export function LearningAids({ lesson, panoId, lat, lng, countryCode, adviceOpen, refreshKey, onAdviceClose, onSaveClue, onNoteSaved }: {
  ...
  return <>
    ...
    {open === 'available' && <aside ref={panelRef} style={dragStyle} className="learning-aid-panel available-notes-panel" aria-label={t('Available notes')}>
      ...
      <div className="available-notes-list">{notes.map((item) => <article key={item.id}>{item.imageUrl && <img src={item.imageUrl} alt="" onError={() => setNotes((current) => current.map((note) => note.id === item.id ? { ...note, imageUrl: undefined, missingImage: true } : note))} />}<strong>{item.category ? `${t(item.category)} · ` : ''}{item.source}</strong>{item.missingImage && <p className="clue-integrity-error">{t('Saved photo is missing. Keep this note; recovery may still restore it.')}</p>}{item.text && <p><CoachRichText text={item.text} /></p>}{item.analysis && analysisDetails(item.analysis)}<small>{new Date(item.at).toLocaleString(ui)}</small></article>)}</div>
    </aside>}
