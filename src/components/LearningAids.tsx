...
export function LearningAids({ lesson, panoId, lat, lng, countryCode, adviceOpen, refreshKey, onAdviceClose, onSaveClue, onNoteSaved }: {
  ...
  const updateNote = (text: string) => { setNote(text); setNoteSaved(false); void trainerDb.setSetting('workspace.noteDraft', { panoId, text, category } satisfies NoteDraft); };
  ...
  return <>
    ...
    {open === 'available' && <aside ref={panelRef} style={dragStyle} className="learning-aid-panel available-notes-panel" aria-label={t('Available notes')}>
      ...
      <div className="available-notes-list">{notes.map((item) => <article key={item.id}>{item.imageUrl && <img src={item.imageUrl} alt="" />}<strong>{item.category ? `${t(item.category)} · ` : ''}{item.source}</strong>{item.text && <p><CoachRichText text={item.text} /></p>}{item.analysis && analysisDetails(item.analysis)}<small>{new Date(item.at).toLocaleString(ui)}</small></article>)}</div>
    </aside>}
