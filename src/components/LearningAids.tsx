...
type AvailableNote = { id: string; source: string; text: string; category?: string; imageUrl?: string; missingImage?: boolean; analysis?: CoachAnalysis; at: number; notebook?: NotebookNote; coach?: CoachHistoryNote; clueId?: string };
...
export function LearningAids({ lesson, panoId, lat, lng, countryCode, adviceOpen, refreshKey, onAdviceClose, onSaveClue, onNoteSaved }: {
  ...
  useEffect(() => { let active = true; void Promise.all([trainerDb.setting<NotebookNote[]>('notebook.notes'), trainerDb.setting<CoachHistoryNote[]>('coach.notes'), trainerDb.attempts(), trainerDb.studyVisits(), trainerDb.clues(), trainerDb.locations()]).then(([personal = [], coach = [], attempts, visits, savedClues, locations]) => { if (!active) return;
    ...
    const currentNotes: AvailableNote[] = [...displayedPersonal.map((item) => { const linked = visibleNearbyClues.find((clue) => clue.id === personalLinks.get(item)); return { id: item.id || `personal:${item.updatedAt}`, source: t('Personal'), text: item.text, category: item.category, imageUrl: linked?.imageDataUrl, missingImage: !!item.clueId && (!linked || !linked.imageDataUrl), analysis: linked && (linked.analysis.description || linked.analysis.strongClues.length) ? linked.analysis : undefined, at: item.updatedAt, notebook: item, clueId: linked?.id }; }), ...[...coachByRun.values()].map((item) => ({ id: item.id, source: t('AI-assisted'), text: item.analysis.description || '', imageUrl: item.clueId ? visibleNearbyClues.find((clue) => clue.id === item.clueId)?.imageDataUrl : undefined, analysis: item.analysis, at: item.generatedAt, coach: item, clueId: item.clueId })), ...visibleNearbyClues.filter((item) => !linkedClues.has(item.id)).map((item) => ({ id: item.id, source: t(item.origin === 'personal' ? 'Personal' : 'AI-assisted'), text: item.analysis.description || '', imageUrl: item.imageDataUrl, analysis: item.analysis, at: item.createdAt, clueId: item.id }))];
