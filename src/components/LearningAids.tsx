...
export function LearningAids({ lesson, panoId, lat, lng, countryCode, adviceOpen, refreshKey, onAdviceClose, onSaveClue, onNoteSaved }: {
  ...
  const [notes, setNotes] = useState<Array<{ id: string; source: string; text: string; category?: string; imageUrl?: string; missingImage?: boolean; analysis?: CoachAnalysis; at: number }>>([]);
  ...
  const updateNote = (text: string) => { setNote(text); setNoteSaved(false); void writeWorkspaceDraft('note', { panoId, text, category } satisfies NoteDraft); };
  ...
  return <>
    ...
    {open === 'available' && <aside ref={panelRef} style={dragStyle} className="learning-aid-panel available-notes-panel" aria-label={t('Available notes')}>
      ...
      <div className="available-notes-list">{notes.map((item) => <article key={item.id}>{item.imageUrl && <img src={item.imageUrl} alt="" />}<strong>{item.category ? `${t(item.category)} · ` : ''}{item.source}</strong>{item.missingImage && <p className="clue-integrity-error">{t('Saved photo is missing. Keep this note; recovery may still restore it.')}</p>}{item.text && <p><CoachRichText text={item.text} /></p>}{item.analysis && analysisDetails(item.analysis)}<small>{new Date(item.at).toLocaleString(ui)}</small></article>)}</div>
    </aside>}
