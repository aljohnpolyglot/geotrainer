...
export function ClueGallery({ country, clues, initialSelectedId, onClose, onDelete }: { country: string; clues: ClueRecord[]; initialSelectedId?: string; onClose: () => void; onDelete: (id: string) => void }) {
  ...
  return createPortal(<div className="clue-gallery-backdrop"><aside className={`clue-gallery${selected ? ' detail-open' : ''}`} aria-label={`${displayCountry} ${t('knownClues')}`}>
    ...
    {selected ? <div className="clue-gallery-detail">{location ? <div className="clue-gallery-streetview"><StreetViewContainer currentLocation={location} isLoading={false} onNextLocation={() => {}} canMove canPan canZoom showCompass={false} /><img className="clue-gallery-reference" src={selected.imageDataUrl} alt={t('savedVisualClue')} /></div> : <img src={selected.imageDataUrl} alt={t('savedVisualClue')} />}
      {selected.analysis.style && <p className="coach-history-profile"><strong>{coachStyleLabel(selected.analysis.style)}</strong>{selected.analysis.depth && <span>{t(selected.analysis.depth[0].toUpperCase() + selected.analysis.depth.slice(1))}</span>}</p>}{selected.analysis.description && <p>{selected.analysis.description}</p>}
