...
export function ClueGallery({ country, clues, initialSelectedId, onClose, onDelete }: { country: string; clues: ClueRecord[]; initialSelectedId?: string; onClose: () => void; onDelete: (id: string) => void }) {
  ...
  const [selected, setSelected] = useState<ClueRecord | null>(() => clues.find((clue) => clue.id === initialSelectedId) || null);
  ...
  return <div className="clue-gallery-backdrop"><aside className="clue-gallery" aria-label={`${country} ${t('knownClues')}`}>
    <header>{selected ? <button className="icon-button" onClick={() => setSelected(null)} aria-label={t('Back')}><ArrowLeft size={17} /></button> : <span />}<div><h2><CountryFlag code={selected?.countryCode || clues[0]?.countryCode} />{country}</h2><p>{selected ? t('Details') : `${clues.length} ${t('knownClues')}`}</p></div><button className="icon-button" onClick={onClose} aria-label={t('close')}><X size={17} /></button></header>
