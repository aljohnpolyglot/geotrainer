...
export function ClueGallery({ country, clues, initialSelectedId, onClose, onDelete }: { country: string; clues: ClueRecord[]; initialSelectedId?: string; onClose: () => void; onDelete: (id: string) => void }) {
  ...
  return createPortal(<div className="clue-gallery-backdrop"><aside className={`clue-gallery${selected ? ' detail-open' : ''}`} aria-label={`${country} ${t('knownClues')}`}>
    <header><span /><div><h2><CountryFlag code={selected?.countryCode || clues[0]?.countryCode} />{country}</h2><p>{selected ? `${t(selected.origin === 'personal' ? 'Personal' : 'AI-assisted')} · ${new Date(selected.createdAt).toLocaleString(ui)}` : `${clues.length} ${t('knownClues')}`}</p></div><button className="icon-button" onClick={onClose} aria-label={t('close')}><X size={17} /></button></header>
