...
For a Supabase-independent production endpoint, deploy this repository to Vercel, set the server-only `GOOGLE_MAPS_API_KEY` and `GEMINI_API_KEYS` environment variables there, then add the resulting `https://<project>.vercel.app/api/coach` URL as the GitHub repository secret `VITE_COACH_ENDPOINT`. Set `COACH_ALLOWED_ORIGINS` only when serving the app from an origin other than the included GitHub Pages site. Redeploy both projects after changing environment variables.
